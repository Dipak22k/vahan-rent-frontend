  ...
  export default function CarDetailsScreen({ route, navigation }) {
    ...
    try {
      ...
      // ✅ KYC CHECK
    // 🔥 ALWAYS GET LATEST USER FROM STORAGE
  // ✅ ALWAYS CHECK FROM BACKEND (NOT LOCAL STORAGE)
  const token = await AsyncStorage.getItem("userToken");

  const kycRes = await fetch(`${CONFIG.BASE_URL}/api/kyc/status`, {
    headers: {
  ...
  if (kycData.status !== "verified") {
    Alert.alert("KYC Required", "Please complete KYC.", [
      { text: "Cancel", style: "cancel" },
      { text: "Go to KYC", onPress: () => navigation.navigate("KYCMaster") }
    ]);
    return;
  }
      ...
      // ❌ FIX: handle 403 properly
      if (res.status === 403) {
        Alert.alert("KYC Required", data?.message || "Complete your KYC first");
        return;
      }
  ...
  otherUser: lender,
      ...
      console.log("CHAT ERROR:", err); // 👈 IMPORTANT DEBUG
